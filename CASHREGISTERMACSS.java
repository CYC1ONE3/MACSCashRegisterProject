...
package com.pogi.cashregistermacss;
...
public class CASHREGISTERMACSS {
    ...
    public static void main(String[] args) {
        ...
        while (anotherTransaction) {
            ...
            while (true) {
                displayMenu();
                System.out.println("\n1. Order  2. View Cart  3. Remove Item  4. Edit Quantity of Order  5. Checkout  6. Exit");
                ...
                switch (choice) {
                    ...
                    case 3:
                        ...
                        break;
                    case 4:
                        editItemQuantity(cartItems, cartItemPrice, cartTotalPrice, cartQuantities);
                        break;
                    case 5:
    ...
    public static void editItemQuantity(ArrayList<String> cartItems, ArrayList<Double> cartItemPrice, ArrayList<Double> cartTotalPrice, ArrayList<Integer> cartQuantities) {
        if (cartItems.isEmpty()) {
            System.out.println("Cart is empty.");
            return;
        }
        ...
        System.out.println("\nYour Cart:");
        for (int i = 0; i < cartItems.size(); i++) {
            System.out.println((i + 1) + ". " + cartItems.get(i) + " | Quantity: " + cartQuantities.get(i) + " | Price: PHP " + cartItemPrice.get(i) + " | Total: PHP " + cartTotalPrice.get(i));
        }

        System.out.print("\nEnter the number of the item you want to update quantity for: ");
        int index = CYC.nextInt();
        CYC.nextLine();

        if (index >= 1 && index <= cartItems.size()) {
            System.out.print("Enter the new quantity for " + cartItems.get(index - 1) + ": ");
            int newQuantity = CYC.nextInt();
            CYC.nextLine();

            if (newQuantity <= 0) {
                System.out.println("Quantity must be at least 1.");
            } else {
                cartQuantities.set(index - 1, newQuantity);
                double newTotal = newQuantity * cartItemPrice.get(index - 1);
                cartTotalPrice.set(index - 1, newTotal);
    ...
    public static void logTransaction(String username, ArrayList<String> cartItems, ArrayList<Integer> cartQuantities, ArrayList<Double> cartItemPrice, double totalAmount, double payment, double change) {
        try (FileWriter sulat = new FileWriter("transactions.txt", true)) {
            ...
            for (int i = 0; i < cartItems.size(); i++) {
                sulat.write(" - " + cartItems.get(i) + " | Quantity: " + cartQuantities.get(i) + " | Price: PHP " + cartItemPrice.get(i) + "\n");
            }
